...
- [ ] No custom `box-shadow` on card divs — use `Card` component
- [ ] No custom toggle switches — use `Switch`
- [ ] No `<button style={{ borderRadius: 9999 }}>` — use `Button` with system radius
- [ ] No nested `ActionArea` inside `ActionArea`
...
- [ ] No importing `lightColors`/`darkColors` for inline styles — use CSS variables
- [ ] No `success`/`warning`/`info` colors for general-purpose buttons — use for status only
- [ ] No two `color="primary"` buttons side by side — one primary, one neutral weak
- [ ] No `var(--surface-base-container)` for section/page backgrounds — use `--surface-base-default` or `--surface-base-alternative`
...
- [ ] No arbitrary spacing values like `gap: 13` — use token multiples of 4
- [ ] No full-screen loading spinners — use inline skeleton states
- [ ] No blank white sections when data is empty — always show empty state
- [ ] No destructive actions placed next to primary CTAs without visual separation
- [ ] No fixed pixel widths for buttons — let content or container define width
...
- [ ] No `onClick` on `<div>`/`<span>` without `role="button"` and keyboard handler
- [ ] Color is not the only indicator of state — add text or icon alongside
- [ ] No removed focus outlines without replacement
- [ ] No `tabIndex={-1}` on elements that should be keyboard-accessible

## Motion
- [ ] No transition durations over 500ms for UI feedback
- [ ] No custom `transition: all` — use `transitions.all` from token system
- [ ] No animating layout properties (`width`, `height`) — causes reflow
